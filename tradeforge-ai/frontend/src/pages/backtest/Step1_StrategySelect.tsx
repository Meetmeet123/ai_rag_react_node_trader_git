...
import { BACKTEST_STRATEGIES } from './mockData';
...
export default function Step1_StrategySelect({ selectedId, onSelect, onNew }: Step1Props) {
  return (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {BACKTEST_STRATEGIES.map((strategy) => (
          ...
          >
            ...
            <p className="text-[13px] text-[#94A3B8] mb-3 line-clamp-2 leading-relaxed">{strategy.description}</p>
            <div className="flex items-center gap-2">
              <span className="text-[11px] font-medium px-2 py-0.5 rounded-full bg-[#1A1A25] text-[#94A3B8]">
                {strategy.segment}
              </span>
              <span className="text-[11px] text-[#475569]">Last edited: {strategy.lastModified}</span>
            </div>
          </button>
        ))}
